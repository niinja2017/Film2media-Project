...
export default function Navbar({ changePageHandler }) {
  ...
  // اگر خواستی با کلید Esc منو بسته بشه
  useEffect(() => {
    const onKey = (e) => {
      if (e.key === "Escape") {
        setCatOpen(false);
                          ...
                          <li key={ci}>
                            <button
                              ...
                              className={`w-full cursor-pointer text-center py-2 rounded font-semibold block transition-colors duration-200
                                ${activeCategoryIndex === ci ? "bg-[#96f207] text-[#0d0d0d]" : "bg-[#121d01] text-[#96f207] hover:bg-[#96f207] hover:text-[#121d01]"}
                              `}
                            >
                              {cat.title}
                            </button>
                      ...
                      <div className="flex-1">
                        <ul className="flex flex-wrap space-y-3">
                          {categories[activeCategoryIndex].subItems.length > 0 ? (
                            categories[activeCategoryIndex].subItems.map((sub, si) => (
                              <li key={si} className="w-1/4">
                                <a
                                  href="#"
